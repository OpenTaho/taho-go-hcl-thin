...
func (h *HclThinParser) NewFile(name string) hcl.HclFile {
	...
	fileNode.SetBody(readNodes(fileNode, 0, br, echo, fileNode.Body(), false))

	body = append(body, fileNode)
...
func readNodes(parent hcl.HclNode, endRune rune, br *bufio.Reader, echo *strings.Builder, nodes []hcl.HclNode, exitOnEOL bool) []hcl.HclNode {
	c := HclUnknown
	...
	for {
		if stringNext {
			...
			stringNodes, err = readString(br, echo, &sb, parent, stringNodes)
			if err != nil {
				panic(err)
			}
			ignore = true
		...
		if r == '(' || r == '{' || r == '[' {
			...
			n.SetBody(readNodes(n, endRuneMap[r], br, echo, []hcl.HclNode{}, false))
			nodes = append(nodes, n)
		} else if r == endRune {
			if sb.Len() > 0 {
				nodes = addNode(parent, nodes, hclType, sb.String())
			}
			willBreak = true
		} else if isToken(r) {
			...
			if p[0] == '=' {
				...
				if last > 0 {
					...
					if strings.TrimSpace(v) != "" {
						nodes = addNode(parent, nodes, hclType, v)
						last++
					}
					n := nodes[last]
					n.SetType(hcl.HclTypePair)
					n.SetBody(readNodes(n, 0, br, echo, n.Body(), true))
					body := n.Body()
			...
			if strings.TrimSpace(sb.String()) == "" && sb.String() != "" {
				nodes = addNode(parent, nodes, hcl.HclTypeSpace, sb.String())
				sb.Reset()
			}
...
func addNode(parent hcl.HclNode, nodes []hcl.HclNode, hclType hcl.HclType, v string) []hcl.HclNode {
	nLen := len(nodes)
	last := nLen - 1
...
func readString(br *bufio.Reader, echo *strings.Builder, sb *strings.Builder, parent hcl.HclNode, nodes []hcl.HclNode) ([]hcl.HclNode, error) {
	escape := false
	interlop := '\x00'
	for {
		r, _, err := br.ReadRune()
		...
		if r == '\\' {
			...
			interlop = 0
			newEcho := &strings.Builder{}
			nodes = readNodes(parent, '}', br, newEcho, nodes, false)
			echo.WriteString(newEcho.String())
			sb.WriteRune('{')
